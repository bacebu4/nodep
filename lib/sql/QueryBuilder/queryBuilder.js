...
module.exports = {
  ...
  delete({ tableName, where }) {
    const { clause, args } = new WhereClauseBuilder(where);
  ...
  update({ tableName, delta, where }) {
    ...
    const { clause: whereClause, args: whereArgs } = new WhereClauseBuilder(
      where,
      whereArgOffset
    );
  ...
  select({ tableName, where, fields = ['*'] }) {
    ...
    const { clause: whereClause, args } = new WhereClauseBuilder(where);
